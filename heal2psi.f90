...
integer(i4b) :: nres,nside,lmax,hporder,hpspin,nmaps,iter,narg
integer(i8b) :: npix,ipring,ipnest,ipix,np,npixtot,l,m
...
!        read(arg,*)nside  ! HEALPix nside input maps 
    case ('-lmax')
        read (arg,'(I)') lmax   ! HEALPix lmax to do SHTs
    case ('-outfile')
...
call alm2map_der(nside,lmax,lmax,hpalm,hpmap_phi,hpmap_alp,hpmap_mag)
deallocate(hpmap_phi)
deallocate(hpalm)

write(*,*)'*** hpmap_alp(0:npix-1,1:2) => hpmap(1:2,0:npix-1)'
allocate(hpmap(1:2,0:npix-1))
